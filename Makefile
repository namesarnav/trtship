...
.PHONY: help install install-cpu lint format format-check typecheck test test-cpu test-cov test-gpu schema check clean
...
test-gpu:  ## Hardware tests; fails if any were skipped (needs a GPU, TensorRT, Docker, TRTSHIP_TRITON_IMAGE)
	$(BIN)/python -m pytest -m "gpu or tensorrt or triton" -rs --junitxml=gpu-results.xml
	$(BIN)/python scripts/require_no_skips.py gpu-results.xml
...
clean:  ## Remove caches and build output
	rm -rf .pytest_cache .mypy_cache .ruff_cache .coverage htmlcov dist build gpu-results.xml
